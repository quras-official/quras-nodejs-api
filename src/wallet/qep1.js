/**
 * QEP1 - Private Key Encryption based on AES.
...
import { DEFAULT_SCRYPT, QEP_HEADER, QEP_FLAG } from '../consts'
...
/**
 * Encrypts a QTP1 key using a given keyphrase under QEP1 Standard.
 * @param {string} qtpKey - QTP1 key to encrypt (52 chars long).
 * @param {string} keyphrase - The password will be encoded as UTF-8 and normalized using Unicode Normalization Form C (NFC).
 * @param {scryptParams} [scryptParams] - Parameters for Scrypt. Defaults to MEP1 specified parameters.
 * @returns {string} The encrypted key in Base58 (Case sensitive).
 */
export const encrypt = (qtpKey, keyphrase, scryptParams = DEFAULT_SCRYPT) => {
  ...
  const account = new Account(qtpKey)
  ...
  // Construct
  const assembled = QEP_HEADER + QEP_FLAG + addressHash + encrypted.ciphertext.toString()
 ...
 * @param {string} qtpKey - QTP1 key to encrypt (52 chars long).
 * @param {string} keyphrase - The password will be encoded as UTF-8 and normalized using Unicode Normalization Form C (NFC).
 * @param {scryptParams} [scryptParams] - Parameters for Scrypt. Defaults to QEP1 specified parameters.
 * @returns {string} The encrypted key in Base58 (Case sensitive).
 */
export const encryptAsync = (qtpKey, keyphrase, scryptParams = DEFAULT_SCRYPT) => {
...
/**
 * Decrypts an encrypted key using a given keyphrase under QEP1 Standard.
 * @param {string} encryptedKey - The encrypted key (58 chars long).
 * @param {string} keyphrase - The password will be encoded as UTF-8 and normalized using Unicode Normalization Form C (NFC).
 * @param {scryptParams} [scryptParams] - Parameters for Scrypt. Defaults to QEP1 specified parameters.
 * @returns {string} The decrypted QTP1 key.
 */
