...
import { isPublicKey, isPrivateKey, isQTP1 } from './verify'
...
export const signMessage = (message, privateKey) => {
  if (!isPrivateKey(privateKey) && !isQTP1(privateKey)) throw new Error('Invalid private key or WIF')
  if (isQTP1(privateKey)) privateKey = getPrivateKeyFromWIF(privateKey)
