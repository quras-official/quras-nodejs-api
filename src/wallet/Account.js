...
import { isPrivateKey, isPublicKey, isQTP1, isAddress, isQEP1, isScriptHash } from './verify'
import { encrypt, decrypt } from './qep1'
 ...
 * QEP1 <=> QTP1 <=> Private => Public => ScriptHash <=> Address
 * @param {string|object} str - QTP1/ Private Key / Public Key / Address or a Wallet Account object.
 */
class Account {
  ...
  /**
   * Key encrypted according to QEP1 standard.
   * @type {string}
   */
  get encrypted () {
    if (this._encrypted) {
  ...
  get QTP1 () {
    if (this._QTP1) {
      ...
      this._QTP1 = core.getWIFFromPrivateKey(this._privateKey)
      return this._QTP1
    }
  }
  ...
  get privateKey () {
    if (this._privateKey) {
      ...
      this._privateKey = core.getPrivateKeyFromWIF(this._QTP1)
      return this._privateKey
  ...
  decrypt (keyphrase, scryptParams = undefined) {
    this._QTP1 = decrypt(this.encrypted, keyphrase, scryptParams)
    this._updateContractScript()
    return this
  }
