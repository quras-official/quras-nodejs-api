 ...
 * @return {boolean}
 */
export const isQEP1 = (qep1) => {
  try {
    if (qep1.length !== 58) return false
    const hexStr = ab2hexstring(base58.decode(qep1))
    if (!hexStr) return false
    if (hexStr.length !== 86) return false
    if (hexStr.substr(0, 2) !== '00') return false
    if (hexStr.substr(2, 2) !== '02') return false
    if (hexStr.substr(4, 2) !== '19') return false
    return true
 ...
 * @return {boolean}
 */
export const isQTP1 = (qtp1) => {
  try {
    if (qtp1.length !== 52) return false
    const hexStr = ab2hexstring(base58.decode(qtp1))
...
export const isAddress = (address) => {
  try {
    ...
    if (getAddressFromScriptHash(scriptHash) !== address) {
      // address is not valid Quras address, could be btc, ltc etc.
      return false
    }
    return true
  } catch (e) { return false }
