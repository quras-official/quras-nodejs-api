import { StringStream, num2hexstring, reverseHex, ensureHex, int2hex, str2ab, ab2hexstring, str2hexstring, hexstring2str } from '../utils.js'
...
class ScriptBuilder extends StringStream {
   ...
   * @return {ScriptBuilder} this
   */
  emitSysCall (api, addLengthFlag = 1) {
    ...
    if (addLengthFlag === 0) {
      out = apiBytes
      return this.emit(OpCode.SYSCALL - 1, out)
    }
  ...
  while (!sb.isEmpty()) {
    ...
    switch (true) {
      ...
      case (n < 75):
        ...
        break
      case (n >= 76 && n <= 78):
        let length = parseInt(sb.read(Math.pow(2, n - 76)), 16)
        output.args.unshift(sb.read(length))
        break
      case (n >= 81 && n <= 96):
      ...
      case (n === 104):
        var blen = sb.read(1)
        var nlen = parseInt(blen, 16)
        var api = sb.read(nlen)
        output.SysCall = hexstring2str(api)
        return output
      case (n === 105):
        output.scriptHash = reverseHex(sb.read(20))
