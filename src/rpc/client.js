...
import { RPC_VERSION, DEFAULT_RPC, QURAS_NETWORK } from '../consts'
import logger from '../logging'
import { timeout } from '../settings'

const log = logger('rpc')

const versionRegex = /PURE:(\d+\.\d+\.\d+)/
/**
 * @class RPCClient
 * @classdesc
 * RPC Client model to query a Quras node. Contains built-in methods to query using RPC calls.
 * @param {string} net - 'MainNet' or 'TestNet' will query the default RPC address found in consts. You may provide a custom URL.
 * @param {string} version - Version of QURAS node. Used to check if RPC methods have been implemented. it will default to DEFAULT_RPC found in CONST
 */
class RPCClient {
  constructor (net, version = RPC_VERSION) {
    if (net === QURAS_NETWORK.MAIN) {
      this.net = DEFAULT_RPC.MAIN
    } else if (net === QURAS_NETWORK.TEST) {
      this.net = DEFAULT_RPC.TEST
    } else if (net === QURAS_NETWORK.DEV) {
      this.net = DEFAULT_RPC.DEV
    } else {
      /**
   ...
   * Gets a list of all transaction hashes waiting to be processed.
   * @param {string} scriptHash
   * @return {Promise<JArray[]>}
   */
  getMemPoolCoin (scriptHash) {
    return this.execute(Query.getMemPoolCoin(scriptHash))
      .then((res) => {
        return res.result
      })
  }
   ...
   * @return {Promise<JArray[]>}
   */
  getMemPoolTransaction (scriptHash) {
    return this.execute(Query.getMemPoolTransaction(scriptHash))
      .then((res) => {
        return res.result
      })
  }
  ...
  /**
   * Gets the version of the QURAS node. This method will never be blocked by version. This method will also update the current Client's version to the one received.
   * @return {Promise<string>}
   */
  getVersion () {
  ...
  invoke (scriptHash, params) {
    if (semver.lt(this.version, '2.3.2')) return Promise.reject(new Error('This method is not implemented for this version'))
  ...
  getClaimAmount (params) {
    if (semver.lt(this.version, '2.3.2')) return Promise.reject(new Error('This method is not implemented for this version'))
    return this.execute(Query.getClaimAmount(params))
      .then((res) => {
        return res.result
      })
  }
  ...
  /**
   * Checks if the provided address is a valid QURAS address.
   * @param {string} addr
   * @return {Promise<boolean>}
   */
