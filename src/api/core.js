 ...
 * The core functionality are sendAsset, claimXQG and doInvoke.
 ...
 * @param {string} config.net - 'MainNet', 'TestNet' or a qurasDB URL.
...
/**
 * Perform a ClaimTransaction for all available XQG based on API
 * @param {object} config - Configuration object.
 * @param {string} config.net - 'MainNet', 'TestNet' or string.
 * @param {string} config.address - Wallet address
...
export const claimXQG = config => {
  return fillUrl(config)
    ...
    .catch(err => {
      ...
      log.error(`claimXQG failed with ${err.message}. Dumping config`, dump)
      throw err
    })
}
...
export const createTx = (config, txType) => {
  ...
  switch (txType) {
    ...
    case 1:
      checkProperty(config, 'balance', 'intents')
      tx = Transaction.createIssueTx(config.balance, config.intents, config.override, config.fees)
      break
    case 'invocation':
    case 209:
 ...
 * @param {string} config.url - QURAS Node URL.
 ...
 * @param {number} assetAmts.XQC - Amt of XQC to send.
 * @param {number} assetAmts.XQG - Amt of XQG to send.
 ...
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint API object. eg, qurasDB.
...
/**
 * Helper method to get the maximum amount of gas claimable after spending all XQC.
 * @param {object} config - Configuration object.
 * @param {string} config.net - 'MainNet', 'TestNet'
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint APi object. eg, qurasDB.
